        ...
        private static void Simulate(CellState[][] current, CellState[][] next)
        {
            for (int y = 0; y < current.Length; y++)
            {
                for (int x = 0; x < current[y].Length; x++)
                {
                    ...
                    else if (current[y][x] is CellState.TakenSeat && neighborCount >= 5)
            ...
            static int getNeighborCount(CellState[][] layout, int posX, int posY)
            {
                ...
                for (int dirY = -1; dirY <= 1; dirY++)
                {
                    ...
                    {
                        if (canSeeTakenSeat(layout, posX, posY, dirX, dirY))
                        {
                            count++;
                        }
                    }
            ...
            static bool canSeeTakenSeat(CellState[][] layout, int posX, int posY, int dirX, int dirY)
            {
                // Direction is 0.
                if (dirX == 0 && dirY == 0)
                {
                ...
                while (true)
                {
                    ...
                    // Handle no neighbor.
                    if (posY < 0 || posY >= layout.Length || posX < 0 || posX >= layout[posY].Length)
                    {
                        return false;
                    }

                    var state = layout[posY][posX];
                    if (state == CellState.TakenSeat)
                        return true;
                    else if (state == CellState.FreeSeat)
                        return false;
                }
            }
        }
