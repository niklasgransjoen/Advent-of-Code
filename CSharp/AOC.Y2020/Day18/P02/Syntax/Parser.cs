...
namespace AOC.Y2020.Day18.P02.Syntax
{
    internal class Parser
    {
        ...
        private ExpressionSyntax Parse()
        {
            var left = ParsePrimitive();
            while (true)
            {
                if (_currentIndex >= _tokens.Count || Current.Kind == TokenKind.CloseParenthesisToken)
                    return left;
        ...
        {
            return Current.Kind == TokenKind.NumberToken ? 
        ...
        private ExpressionSyntax ParseParenthesisedExpression()
        {
            ...
            var parenthesisedExpression = Parse();
